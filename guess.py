...
def classify_one_multi_crop(sess, label_list, softmax_output, coder, images, image_file, writer):
    try:
        ...
        print('Guess @ 1 %s, prob = %.2f' % best_choice)
        # self-implemented
        # if the subject falls into the age (25, 32) with a prob of 0.79, 
        # the age_est would be 32 * 0.79 = 25.28, in order to easily compare 
        # the age among subjects!
        str_best = label_list[best]
        str_b = str_best.split(" ")
        str_b_partial = str_b[1].split(")")
        flo_best = float(str_b_partial[0])
        age_est = flo_best * output[best]
        print('Age_est = %.2f' % age_est)
        ...
        if nlabels > 2:
            ...
            str_best2 = label_list[second_best]
            str_b2 = str_best2.split(" ")
            str_b_partial2 = str_b2[1].split(")")
            flo_best2 = float(str_b_partial2[0])
            age_est2 = flo_best2 * output[second_best] + age_est
            print('Age_est_2 = %.2f' % age_est2)
